...
export function ProfilePage() {
  const { user, authLoading, logout, fragrances, wishlistIds, toggleWishlist, openSourceModal, showToast, saveQuizResult } = useApp();
  ...
  return (
    <>
      ...
      <div className="profile-layout">
        <WishlistPanel fragrances={fragrances} wishlistIds={wishlistIds} toggleWishlist={toggleWishlist} openSourceModal={openSourceModal} />
        <QuizPanel
...
function WishlistPanel({ fragrances, wishlistIds, toggleWishlist, openSourceModal }) {
  ...
  return (
    <div className="profile-panel">
      ...
      <p className="profile-panel-sub">Fragrances you've saved. Request sourcing on any of them.</p>
      {items.length === 0 ? (
          ...
          {items.map((p, i) => (
            <div key={p.id} className="wishlist-item">
              ...
              <div className="wishlist-item-actions">
                <button type="button" className="wishlist-action source" onClick={() => openSourceModal(`${p.name} — ${p.brand}`)}>Source</button>
                <button type="button" className="wishlist-action remove" onClick={() => toggleWishlist(p.id)}>✕</button>
              </div>
            </div>
          ))}
        </div>
...
function QuizPanel({ savedResult, onSave, openSourceModal }) {
      ...
      {phase === 'result' && result && (
        <div className="quiz-result">
          ...
          <div className="quiz-result-actions">
            <button type="button" className="btn-dark" onClick={() => openSourceModal(`${result.name} — ${result.brand}`)}>Source This Bottle</button>
